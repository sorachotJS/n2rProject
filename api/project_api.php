<?php
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        ...
        if($method == "POST"){
            ...
            $remark1 = '';
            $remark2 = '';
            $sql = "insert into t_project(Title,Category,Img_url,Remark1,Remark2) values('$title','$category','$img_url','$remark1','$remark2')";
            $stmp = $conn->prepare($sql);
            $stmp->execute();
        ...
        if($method == "DELETE"){
            $idd = $_REQUEST['idd'];
            $sql = "DELETE FROM t_project WHERE ID = '$idd'";
            $stmp = $conn->prepare($sql);
            $stmp->execute();
    
            $sqlsel = "select * from t_project where ID='$idd'";
            $stmt = $conn->prepare($sqlsel);
            $stmt->execute();
            $result = $stmt->fetchAll();
